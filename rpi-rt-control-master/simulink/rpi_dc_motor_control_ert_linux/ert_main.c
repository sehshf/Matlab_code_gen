...
#include "rpi_dc_motor_control.h"      /* Model's header file */
...
void * base_rate()
{
  ...
  struct timespec period = { 0U, 1000000U };/* 0.001 seconds */
  ...
  while (rtmGetErrorStatus(rpi_dc_motor_control_M) == NULL &&
         !rtmGetStopRequested(rpi_dc_motor_control_M)) {
    /* Check subrate overrun, set rates that need to run this time step*/

    /* Trigger sub-rate threads */

    /* Execute base rate step */
    rpi_dc_motor_control_step();
    do {
      ...
      if (now.tv_sec > next.tv_sec ||
        ...
        uint32_T usec = (now.tv_sec - next.tv_sec) * 1000000 + (now.tv_nsec -
          next.tv_nsec)/1000;
        fprintf(stderr, "Base rate (0.001s) overrun by %d us\n", usec);
        next = now;
        continue;
      }
    } while (0);
...
int_T main(int_T argc, const char_T *argv[])
{
  ...
  /* Initialize model */
  rpi_dc_motor_control_initialize();
  ...
  /* Terminate model */
  rpi_dc_motor_control_terminate();
  errStatus = rtmGetErrorStatus(rpi_dc_motor_control_M);
...
/* Local Variables: */
/* compile-command: "make -f rpi_dc_motor_control.mk" */
/* End: */
